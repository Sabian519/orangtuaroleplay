...
Bisnis_Save(id)
{
    ...
    {
        format(cQuery, sizeof cQuery,"%s, employe%d='%s'", cQuery, z, bEmploy[id][z]);
    }
    format(cQuery, sizeof cQuery,"%s WHERE id = %d", cQuery, id);
...
Bisnis_Reset(id)
{
    ...
    format(bEmploy[id][0], MAX_PLAYER_NAME, "-");
    format(bEmploy[id][1], MAX_PLAYER_NAME, "-");
    format(bEmploy[id][2], MAX_PLAYER_NAME, "-");
	bData[id][bLocked] = 1;
    bData[id][bMoney] = 0;
	bData[id][bProd] = 0;
	bData[id][bVisit] = 0;
    ...
    for(new z = 0; z < MAX_BISNIS_EMPLOYEE; z++)
    {
        format(bEmploy[id][z], MAX_PLAYER_NAME, "-");
    }
	Bisnis_Refresh(id);
}

IsBisnisEmploye(playerid, id)
{
    if(!strcmp(bEmploy[id][0], pData[playerid][pName], true)) return 1;
    if(!strcmp(bEmploy[id][1], pData[playerid][pName], true)) return 1;
    if(!strcmp(bEmploy[id][2], pData[playerid][pName], true)) return 1;
    return 0;
}
...
Bisnis_Refresh(id)
{
    if(id != -1)
    {
		...
		{
			format(string, sizeof(string), "[ID: %d]\n"WHITE_E"Name: "YELLOW_E"%s\n"WHITE_E"Owned by "LG_E"%s", id, bData[id][bName], bData[id][bOwner]);
			bData[id][bPickup] = CreateDynamicPickup(19133, 23, bData[id][bExtposX], bData[id][bExtposY], bData[id][bExtposZ]+0.2, 0, 0, _, 8.0);
        }
        else
        {
            format(string, sizeof(string), "[ID: %d]\n"YELLOW_E"This bisnis for sell\n{FFFFFF}Location: "AQUA"%s\n{FFFFFF}Price: {FFFF00}%s\n{FFFFFF}Type: {FFFF00}%s\n"WHITE_E"use command '"YELLOW_E"/buy"WHITE_E"' to purchase", id, GetLocation(bData[id][bExtposX], bData[id][bExtposY], bData[id][bExtposZ]), FormatMoney(bData[id][bPrice]), type);
            bData[id][bPickup] = CreateDynamicPickup(19133, 23, bData[id][bExtposX], bData[id][bExtposY], bData[id][bExtposZ]+0.2, 0, 0, _, 8.0);
        }
		...
		format(tstr, 128, "[ID: %d]\n"LG_E"Bisnis buy Point\n"WHITE_E"use '"YELLOW_E"/buy"WHITE_E"' here", id);
		bData[id][bLabelPoint] = CreateDynamic3DTextLabel(tstr, ARWIN, bData[id][bPointX], bData[id][bPointY], bData[id][bPointZ]+0.5, 3.0, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 1, id, bData[id][bInt]);
		
		//bData[id][bCP] = CreateDynamicCP(bData[id][bIntposX], bData[id][bIntposY], bData[id][bIntposZ], 1.0, id, bData[id][bInt], -1, 3.0);
        bData[id][bLabel] = CreateDynamic3DTextLabel(string, ARWIN, bData[id][bExtposX], bData[id][bExtposY], bData[id][bExtposZ]+0.5, 2.5, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 1, 0, 0);
...
function LoadBisnis()
{
	...
	new rows = cache_num_rows(), owner[128], name[128], song[128];
 	if(rows)
  	{
		for(new i; i < rows; i++)
		{
			...
			cache_get_value_name(i, "song", song);
			format(bData[bid][bSong], 128, song);
			cache_get_value_name_int(i, "ph", bData[bid][bPh]);
            for(new z = 0; z < MAX_BISNIS_EMPLOYEE; z++)
            {
                new str[64];
                format(str, sizeof str,"employe%d", z);
                cache_get_value_name(i, str, bEmploy[bid][z]);
            }
			Bisnis_Refresh(bid);
			Iter_Add(Bisnis, bid);
		}
...
CMD:createbisnis(playerid, params[])
{
    ...
    format(bEmploy[bid][0], MAX_PLAYER_NAME, "-");
    format(bEmploy[bid][1], MAX_PLAYER_NAME, "-");
    format(bEmploy[bid][2], MAX_PLAYER_NAME, "-");
	...
	format(bData[bid][bSong], 128, "-");
	bData[bid][bPh] = 0;
	Bisnis_Type(bid);
    Bisnis_Refresh(bid);
	Iter_Add(Bisnis, bid);

	mysql_format(g_SQL, query, sizeof(query), "INSERT INTO bisnis SET ID='%d', owner='%s', price='%d', type='%d', extposx='%f', extposy='%f', extposz='%f', extposa='%f', name='%s', song='%s', ph='%d'", bid, bData[bid][bOwner], bData[bid][bPrice], bData[bid][bType], bData[bid][bExtposX], bData[bid][bExtposY], bData[bid][bExtposZ], bData[bid][bExtposA], bData[bid][bName], bData[bid][bSong], bData[bid][bPh]);
...
CMD:editbisnis(playerid, params[])
{
    ...
    //tot
    else if(!strcmp(type, "song", true))
    {
        new song[128];

        if(sscanf(string, "s[128]", song))
            return Usage(playerid, "/editbisnis [id] [owner] [player name] (use '-' to no owner)");
        ...
        Bisnis_Save(bid);
		Bisnis_Refresh(bid);
        SendAdminMessage(COLOR_RED, "%s has adjusted the song of bisnis ID: %d to %s", pData[playerid][pAdminname], bid, song);
    }
...
CMD:lockbisnis(playerid, params[])
{
	foreach(new bid : Bisnis)
	{
		...
		{
			if(!Player_OwnsBisnis(playerid, bid) && !IsBisnisEmploye(playerid, bid)) return Error(playerid, "You don't owner or employed this bisnis.");
...
{
	if(!IsPlayerInRangeOfPoint(playerid, 3.0, 358.6365,181.6947,1008.3828)) return Error(playerid, "Anda harus berada di City Hall!");
...
CMD:mybis(playerid)
{
	...
	ShowPlayerDialog(playerid, DIALOG_MY_BISNIS, DIALOG_STYLE_LIST, "{FF0000}SouthLuxury:RP {0000FF}Bisnis", CMDSString, "Select", "Cancel");
	return 1;
}
...
CMD:bm(playerid, params[])
{
	...
	if(!Player_OwnsBisnis(playerid, pData[playerid][pInBiz]) && !IsWorkshopEmploye(playerid, pData[playerid][pInBiz])) return Error(playerid, "You don't owner or employed this bisnis.");
