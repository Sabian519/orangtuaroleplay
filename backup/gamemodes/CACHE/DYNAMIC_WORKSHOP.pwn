...
Workshop_Refresh(id)
{
    if(id > -1)
    {
        ...
        format(str, sizeof str,"[Workshop ID:%d]\n"WHITE_E"Workshop Price: "LG_E"%s{ffffff}\n{ffffff}Type use "YELLOW_E"'/buy' "WHITE_E"to purchased this Workshop", id, FormatMoney(wsData[id][wPrice]));

        if(strcmp(wsData[id][wOwner], "-", true))
            format(str, sizeof str,"[Workshop ID:%d]\n{ffffff}Workshop Name: "YELLOW_E"%s\n{ffffff}Workshop Owner: "YELLOW_E"%s\n{ffffff}STATUS: %s", id, wsData[id][wName], wsData[id][wOwner], stats);
