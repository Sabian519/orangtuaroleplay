...
CMD:credits(playerid)
{
	...
	strcat(line3, ""AQUA"Server Founder & Developer: "YELLOW_E"Garaaaaaaaa.\n");
	strcat(line3, ""RED_E"Support Developer: "YELLOW_E"J4K(Jecki)\n");
	strcat(line3, ""LB_E"Server Manager: "YELLOW_E"MasEka(Eka)\n");
	strcat(line3, ""LB_E"Discord Handler: "YELLOW_E"Cyfaa\n\n");
	format(line2, sizeof(line2), ""LB_E"Server Support: "YELLOW_E"%s & All SA-MP Team\n\n\
	"X1_BLUE"Terima kasih telah bergabung dengan kami! Copyright © 2020 | Gara Anggara Arasyid.", pData[playerid][pName]);
	format(line1, sizeof(line1), "%s%s", line3, line2);
   	ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, ""ORANGE_E"Ranum:RP: "WHITE_E"Server Credits", line1, "OK", "");
	return 1;
}
...
CMD:vip(playerid)
{
	...
	"YELLOW_E"Untuk informasi selengkapnya hubungi Garaa Anggara(Server Owner & Founder)!");
	ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, ""ORANGE_E"Ranum:RP "PINK_E"VIP SYSTEM", longstr2, "Close", "");
	return 1;
}
...
{
    new line3[3500];
    strcat(line3, ""RED_E"...:::... "DOOM_"Donate List Ranum Android "RED_E"...:::...\n");
    ...
    strcat(line3, ""WHITE_E"1. NAMA : "RED_E"Gara_Anggara\n");
    strcat(line3, ""WHITE_E"-  Discord: "RED_E"Garaaaaa\n\n");
	
    strcat(line3, ""WHITE_E"2. NAMA : "RED_E"Jeck\n");
    strcat(line3, ""WHITE_E"-  Discord : "RED_E"J4K\n\n");
	...
	ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, ""ORANGE_E"RanumRP: "WHITE_E"DONATE LIST", line3, "Okay", "");
	return 1;
}
...
CMD:time(playerid)
{
	...
	format(line2, sizeof(line2), ""WHITE_E"Paycheck Time: "YELLOW_E"%d remaining\n"WHITE_E"Bank Money: "LG_E"%d\n"WHITE_E"Rekening Bank: "AQUA"%d\n\n"YELLOW_E"Your Online for %d hour(s) %d minute(s) Righ Now", paycheck, pData[playerid][pBankMoney], pData[playerid][pBankRek], pData[playerid][pHours], pData[playerid][pMinutes]);
   	ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, ""AQUA"Ranum RP: "WHITE_E"My Time", line2, "Okay", "");
	return 1;
}
...
CMD:ranumrp(playerid, params[])
{
	if(!IsPlayerInRangeOfPoint(playerid, 3.0, 1635.64, -2334.27, 13.43)) return Error(playerid, "Anda harus berada di bandara untuk mengclaim starterpack!");
    ...
    Info(playerid, "Kamu telah berhasil mengclaim starterpack, kamu mendapat uang sebesar "LG_E"$600,00 "WHITE_E"dan "YELLOW_E"800 Gold VIP");
    pData[playerid][pStarterpack]++;
    pData[playerid][pGold] = 800;
...
CMD:createidcard(playerid, params[])
{
	...
	format(mstr, sizeof(mstr), "{FFFFFF}Nama: "YELLOW_E"%s\n"WHITE_E"Negara: "BLUE_E"San Andreas\n"WHITE_E"Tgl Lahir: "LB_E"%s\n"WHITE_E"Jenis Kelamin: "AQUA"%s\n\n"RED_E"Berlaku hingga 14 hari!", pData[playerid][pName], pData[playerid][pAge], sext);
...
CMD:newage(playerid, params[])
{
	if(!IsPlayerInRangeOfPoint(playerid, 3.0, 361.8575, 173.5981, 1008.3828)) return Error(playerid, "Anda harus berada di City Hall!");
	//if(pData[playerid][pIDCard] != 0) return Error(playerid, "Anda sudah memiliki ID Card!");
	if(GetPlayerMoney(playerid) < 15000) return Error(playerid, "Anda butuh $150,00 untuk mengganti tgl lahir anda!");
...
CMD:buyplate(playerid, params[])
{
	...
	foreach(new i : PVehicles)
	{
		if(vehid == pvData[i][cVeh])
		{
			if(pvData[i][cOwner] == pData[playerid][pID])
			{
				if(GetPlayerMoney(playerid) < 32500) return Error(playerid, "Anda butuh $325,00 untuk membeli Plate baru.");
				GivePlayerMoneyEx(playerid, -32500);
				new rand = RandomEx(1111, 9999);
				format(pvData[i][cPlate], 32, "S:Luxury-%d", rand);
...
CMD:buyinsu(playerid, params[])
{
	...
	foreach(new i : PVehicles)
	{
		if(vehid == pvData[i][cVeh])
		{
			...
			{
				if(GetPlayerMoney(playerid) < 35000) return Error(playerid, "Anda butuh $350,00 untuk membeli Insurance.");
				GivePlayerMoneyEx(playerid, -35000);
				pvData[i][cInsu]++;
				Info(playerid, "Model: %s || Total Insurance: %d || Insurance Price: $350,00", GetVehicleModelName(pvData[i][cModel]), pvData[i][cInsu]);
			}
			else return Error(playerid, "ID kendaraan ini bukan punya mu! gunakan /v my(/mypv) untuk mencari ID.");
		}
	}
...
{
	if(!IsPlayerInRangeOfPoint(playerid, 3.0, 2246.46, -1757.03, 1014.77))
...
CMD:buy(playerid, params[])
{
	...
	//Component
	if(IsPlayerInRangeOfPoint(playerid, 1.2, 854.55, -604.72, 18.42))
	{
	...
	foreach(new id : Workshop)
	{
		...
		{
			if(wsData[id][wPrice] > pData[playerid][pGold]) return Error(playerid, "Not enough money/gold, you can't afford this Workshop.");
