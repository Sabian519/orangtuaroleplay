...
CMD:factionhelp(playerid)
{
	if(pData[playerid][pFaction] == 1)
	{
		...
		strcat(str, ""BLUE_E"SAPD: /takemarijuana /spike /destroyspike /destroyallspike /givelic\n");
...
CMD:givelic(playerid, params[])
{
	...
	if(sscanf(params, "u", otherid))
	{
	    Usage(playerid, "/givelic <ID/Name>");
	    return true;
	}
	...
	pData[otherid][pDriveLicTime] = gettime() + (15 * 86400);
	Info(playerid, "Anda telah memberikan Driving License kepada %s.", ReturnName(otherid));
	Info(otherid, "Officer %s telah memberikan Driving License kepada anda", ReturnName(playerid));
	return 1;
}
...
CMD:takedmvproof(playerid, params[])
{
	if(pData[playerid][pFaction] != 1)
        return Error(playerid, "You must be a sapd officer.");
	if(pData[playerid][pFactionRank] < 2)
	...
	if(sscanf(params, "u", otherid))
	{
	    Usage(playerid, "/takedmvproof <ID/Name>");
	    return true;
	}
	...
	pData[otherid][pDMVProof] = 0;
	Info(playerid, "Anda telah mengambil DMV-Proof milik %s.", ReturnName(otherid));
	Info(otherid, "Officer %s telah mengambil DMV-Proof milik anda", ReturnName(playerid));
	return 1;
}
