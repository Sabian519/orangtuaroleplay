...
CreateDMVPoint()
{
	...
	format(strings, sizeof(strings), "[DMV-Licenses]\nuse command '/taketest' you can take a driving test");
...
CMD:taketest(playerid, params[])
{
    if(IsPlayerInRangeOfPoint(playerid, 1.5, 1491.1300, 1305.9599, 1093.2890))
    {
		...
		if(GetPlayerMoney(playerid) < 20000) return Error(playerid, "Anda butuh $200,00 untuk membuat Driving License.");
		...
		Info(playerid, "kamu telah mengambil test teori dengan harga "LG_E2"%s", FormatMoney(playerid));
	...
	if(dialogid == DIALOG_DSVEH_TESTBASE3)
	{
		if(response)
		{
			if(listitem == 0)
			{
				...
				new DMVCar[2];
				SetPVarInt(playerid, "PTestVeh", DMVCar[0] = CreateVehicle(405,2052.8206,-1903.9745,13.3249,359.6511-180,6,1, 1)); // Car1
		        ...
		        SetPlayerRaceCheckpoint(playerid, 1, 2029.6878, -1929.6832, 13.2046,1962.7469,-1759.2129,13.3828, 5);
