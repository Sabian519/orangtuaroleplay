...
DisplaySalary(playerid)
{
	...
	if(rows) 
	{
		new list[10000], date[1280], info[4700], money, totalduty, gajiduty, totalsal, total;
		new FactionSal = Random(4500, 7500);
		
		totalduty = pData[playerid][pOnDutyTime] + pData[playerid][pTaxiTime];
		if(totalduty > FactionSal)
		{
			gajiduty = FactionSal;
		}
		else
	...
	{
		new list[10000], totalduty, gajiduty;
		new FactionSal = Random(4500, 7500);
