...
GetRentVehicleCost(carid)
{
	...
	if(carid == 509) return 300; //Bike
	if(carid == 510) return 450; //Mt bike
	if(carid == 463) return 850; //Freeway harley
	if(carid == 586) return 700; //Wayfare
	if(carid == 462) return 650; //Faggio

	//Category Kendaraan Trucker
    if(carid == 414) return 3000;  //
	if(carid == 499) return 2950; //
	if(carid == 498) return 2872; //
	if(carid == 455) return 2992; //
	if(carid == 428) return 3203; //
	if(carid == 578) return 3455; //
    if(carid == 403) return 3500;  //
	if(carid == 514) return 3500; //
	if(carid == 515) return 3750; //
 	return -1;
}
