	...
	if(file)
	{
		...
		while(idx < MAX_MODSHOP)
		{
			...
			if(ModsPoint[idx][ModsPos][0])
			{
				...
				format(string, 128, "[M-ID: %d]\n"YELLOW_E"Modshop Point\n{FFFFFF}use '"YELLOW_E"/myov"WHITE_E"' here", idx);
				ModsPoint[idx][ModsText] = CreateDynamic3DTextLabel(string, ARWIN, ModsPoint[idx][ModsPos][0], ModsPoint[idx][ModsPos][1], ModsPoint[idx][ModsPos][2], 10.0, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 0, ModsPoint[idx][ModsVw], ModsPoint[idx][ModsInt], -1, 10.0);
			}
			idx++;
		}
	}
...
CMD:creatempoint(playerid, params[])
{
 	...
 	for(new idx=1; idx<MAX_MODSHOP; idx++)
	{
	    if(!ModsPoint[idx][ModsPos][0])
	    {
	        ...
	        format(String, 128, "[M-ID: %d]\n"YELLOW_E"Modshop Point\n{FFFFFF}use '"YELLOW_E"/myov"WHITE_E"' here", idx);
			ModsPoint[idx][ModsText] = CreateDynamic3DTextLabel(String, ARWIN, X, Y, Z, 10.0, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 0, ModsPoint[idx][ModsVw], ModsPoint[idx][ModsInt], -1, 10.0);
			idx = MAX_MODSHOP;
			SaveModsPoint();
		}
	}
...
CMD:editmpoint(playerid, params[])
{
	...
	if(!strcmp(params, "posisi", true, 6))
	{
		...
		ModsPoint[id][ModsPickup] = CreateDynamicPickup(1274, 23, X, Y, Z, GetPlayerVirtualWorld(playerid), GetPlayerInterior(playerid), -1, 150.0);
  		format(String, 128, "[M-ID: %d]\n"YELLOW_E"Modshop Point\n{FFFFFF}use '"YELLOW_E"/myov"WHITE_E"' here", id);
		ModsPoint[id][ModsText] = CreateDynamic3DTextLabel(String, ARWIN, X, Y, Z, 10.0, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 0, ModsPoint[id][ModsVw], ModsPoint[id][ModsInt], -1, 10.0);
