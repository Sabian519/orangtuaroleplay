...
function EngineStatus(playerid, vehicleid)
{
	if(!GetEngineStatus(vehicleid))
    {
		...
		if(rand == 0)
		{
			...
			Vehicle(playerid, "Engine "GREEN_E"On");
			SendClientMessage(playerid, ARWIN, "VEHICLE: {FFFFFF}You have "LG_E"succesfully {FFFFFF}started the vehicle engine.");
	...
	else
	{
		...
		Vehicle(playerid, "Engine "RED_E"Off");
	...
	foreach(new i : PVehicles)
	{
		if(pvData[i][cOwner] == pData[playerid][pID])
		{
			...
			if(pvData[i][cVeh] != 0)
			{
				...
				for(new z = 0; z < 4; z++)
				{
					DestroyDynamicObject(vtData[pvData[i][cVeh]][z][vtoy_model]);
				}
...
function OnVehBuyPV(playerid, pid, model, color1, color2, cost, Float:x, Float:y, Float:z, Float:a)
{
	...
	SetPlayerPosition(playerid, 1300.18, -1418.44, 13.50, 188.20, 0);
...
function OnVehRentPV(playerid, pid, model, color1, color2, cost, Float:x, Float:y, Float:z, Float:a, rental)
{
	...
	Iter_Add(PVehicles, i);
	OnPlayerVehicleRespawn(i);
	Servers(playerid, "Anda telah menyewa kendaraan seharga $500,00 / one days dengan model %s(%d)", GetVehicleModelName(model), model);
	SetPlayerPosition(playerid, 1300.18, -1418.44, 13.50, 188.20, 0);
	pData[playerid][pBuyPvModel] = 0;
	return 1;
}
...
{
	return callcmd::v(playerid, "park");
}
...
{
	return callcmd::v(playerid, "trunk");
}

CMD:hood(playerid, params[])
{
	return callcmd::v(playerid, "hood");
}
...
{
	if(pData[playerid][IsLoggedIn] == false) return Error(playerid, "You must be login");
	if(pData[playerid][pInjured] >= 1) return Error(playerid, "You dont have is now command!");
	...
	foreach(new i : Parks)
	{
		if(IsPlayerInRangeOfPoint(playerid, 2.3, ppData[i][parkX], ppData[i][parkY], ppData[i][parkZ]))
		{
			...
			if(GetAnyVehiclePark(i) <= 0) return Error(playerid, "This garage not saved this vehicles");
			...
			ShowPlayerDialog(playerid, DIALOG_PICKUPVEH, DIALOG_STYLE_TABLIST_HEADERS,"Parking Garage",location,"Spawn","Cancel");
		}
	}
	// Sapd Vehicle
	...
	|| IsPlayerInRangeOfPoint(playerid, 8.0, 1568.40, -1695.66, 5.89) || IsPlayerInRangeOfPoint(playerid, 8.0, 1569.1587,-1641.0361,28.5788))
	{
		...
		if(pData[playerid][pvSpawnSapd] == 1) return Error(playerid,"Anda sudah mengeluarkan 1 kendaraan.!");

	    new GARA[10000], String[10000];
	    strcat(GARA, "Vehicles Name\tType\n");
		format(String, sizeof(String), "Police Public\tCars\n");// 596
		strcat(GARA, String);
		...
		strcat(GARA, String);
		ShowPlayerDialog(playerid,DIALOG_SAPD_GARAGE, DIALOG_STYLE_TABLIST_HEADERS,"Static Vehicles SA:PD", GARA,"Spawn","Cancel");
	}
	return 1;
}
...
{
	if(pData[playerid][IsLoggedIn] == false) return Error(playerid, "You must be login!");
	if(pData[playerid][pInjured] >= 1) return Error(playerid, "You dont have now is command!");
	...
	if(id > -1)
	{
		if(CountParkedVeh(id) >= 40)
			return Error(playerid, "Garage this Maximum Vehicle!");
		...
		if(!found)
			return Error(playerid, "This vehicles it's not parking!");
	}
	// Sapd Vehicle
	if(IsPlayerInRangeOfPoint(playerid, 8.0, 1568.40, -1695.66, 5.89) || IsPlayerInRangeOfPoint(playerid, 8.0, 1569.1587,-1641.0361,28.5788)
	|| IsPlayerInRangeOfPoint(playerid, 8.0, 1568.40, -1695.66, 5.89) || IsPlayerInRangeOfPoint(playerid, 8.0, 1569.1587,-1641.0361,28.5788))
	{
    	...
    	DestroyVehicle(SAPD_Garages[playerid]);
		pData[playerid][pvSpawnSapd] = 0;
...
CMD:ainsu(playerid, params[])
{
	...
	format(msg2, sizeof(msg2), "ID\tInsurance\tC-ID Insurance\tClaim Time\tPay Ticket\n");
	foreach(new i : PVehicles)
	{
		if(pvData[i][cOwner] == pData[otherid][pID])
		{
			...
			{
				format(msg2, sizeof(msg2), "%s%d\t%s - %d\t%d\t%s\t"LG_E2"$%s\n", msg2, pvData[i][cVeh], GetVehicleModelName(pvData[i][cModel]), pvData[i][cVeh], pvData[i][cInsu], ReturnTimelapse(gettime(), pvData[i][cClaimTime]), FormatMoney(pvData[i][cTicket]));
				found = true;
			}
			else
			{
				format(msg2, sizeof(msg2), "%s%d\t%s - %d\t%d\tNotInsu\t"LG_E2"$%s\n", msg2, pvData[i][cVeh], GetVehicleModelName(pvData[i][cModel]), pvData[i][cVeh], pvData[i][cInsu], FormatMoney(pvData[i][cTicket]));
				found = true;
			}
		}
	}
	if(found) ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_TABLIST_HEADERS, "Insurance Vehicles", msg2, "Close", "");
	else ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, "Vehicles", "Player tidak memeliki kendaraan", "Close", "");
	return 1;
}
...
CMD:v(playerid, params[])
{
	...
	if(sscanf(params, "s[20]S()[128]", type, string))
	{
		...
		SendClientMessage(playerid,COLOR_WHITE,"VEHICLE: /v [my(/mv)] [lock] [park] [modr]");
		return 1;
	}
	if(!strcmp(type, "en", true))
	{
		...
		if(IsPlayerInAnyVehicle(playerid) && GetPlayerState(playerid) == PLAYER_STATE_DRIVER)
		{
			...
			else
			{
				...
				//InfoTD_MSG(playerid, 2000, "Turnon Engine..");
				SCM(playerid, ARWIN, "ENGINE: "WHITE_E"mencoba menyalakan mesin kendaraan");
	...
	else if(!strcmp(type,"hood",true))
	{
        ...
        switch (GetHoodStatus(vehicleid))
        {
        	case false:
        	{
				...
				GameTextForPlayer(playerid, "~w~Vehicle Hood~g~ Opened", 3500, 3);
        	}
        	case true:
        	{
        		SwitchVehicleBonnet(vehicleid, false);
				GameTextForPlayer(playerid, "~w~Vehicle Hood~r~ Close", 3500, 3);
        	}
        }
    }
	...
	{
		new bool:found = false, msg2[10000];
		format(msg2, sizeof(msg2), "ID\tInsurance\tC-ID Insurance\tClaim Time\tPay Ticket\n");
		foreach(new i : PVehicles)
		{
			if(pvData[i][cOwner] == pData[playerid][pID])
	...
	if(!strcmp(type, "modr", true))
	{
		...
		if(IsPlayerInAnyVehicle(playerid) && GetPlayerState(playerid) == PLAYER_STATE_DRIVER)
		{
			...
			for(new i = 0; i < 4; i++)
			{
				DestroyDynamicObject(vtData[vehicleid][i][vtoy_model]);

				vtData[vehicleid][i][vtoy_model] = CreateDynamicObject(vtData[vehicleid][i][vtoy_modelid],
				vtData[vehicleid][i][vtoy_x],
				vtData[vehicleid][i][vtoy_y],
				vtData[vehicleid][i][vtoy_z],
				vtData[vehicleid][i][vtoy_rx],
				vtData[vehicleid][i][vtoy_ry],
				vtData[vehicleid][i][vtoy_rz]);

				AttachDynamicObjectToVehicle(vtData[vehicleid][i][vtoy_model],
				vehicleid,
				vtData[vehicleid][i][vtoy_x],
				vtData[vehicleid][i][vtoy_y],
				vtData[vehicleid][i][vtoy_z],
...
/*SetPrivateVehiclePark(playerid, vehicleid, Float:newx, Float:newy, Float:newz, Float:newangle, Float:health, fuel)
{
	...
	{		
		new vehid;
		if(!IsPlayerInRangeOfPoint(playerid, 3.0, 1299.74, -1435.26, 13.59)) return Error(playerid, "You must in showroom/dealer!");
