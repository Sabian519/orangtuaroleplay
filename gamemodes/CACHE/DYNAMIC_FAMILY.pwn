...
Family_Refresh(id)
{
	if(id != -1)
	{
		...
		{
			format(tstr, sizeof(tstr), "[F-ID: %d]\nSafe Point", id);
...
Family_OpenStorage(playerid, fid)
{
    ...
    for (new i = 0; i < 10; i ++) if(fData[fid][fGun][i]) 
	{
        items[0]++;
    }
    format(string, sizeof(string), "Weapon Storage (%d/10)", items[0]);
 	...
 	if(rows)
  	{
		...
		format(string, sizeof(string), "F-ID: %d\nFamily Name: %s\nFamily Leader: %s\nFamily Marijuana: %d\nFamily Component: %d\nFamily Material: %d\nFamily Money: %s",
