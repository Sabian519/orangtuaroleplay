	...
	if(pData[playerid][pHarvestID] != -1)
	{
		...
		if(pData[playerid][pActivityTime] >= 100)
		{
			if(PlantData[id][PlantType] == 1)
			{
				...
				Info(playerid, "Anda mendapatkan hasil panen potato/kentang seberat "LG_E2"%d kg.", kg);
			}
			else if(PlantData[id][PlantType] == 2)
			{
				pData[playerid][pWheat] += kg;
				Info(playerid, "Anda mendapatkan hasil panen wheat/gandum seberat "LG_E2"%d kg.", kg);
			}
			else if(PlantData[id][PlantType] == 3)
			{
				pData[playerid][pOrange] += kg;
				Info(playerid, "Anda mendapatkan hasil panen orange/jeruk seberat "LG_E2"%d kg.", kg);
			}
			else if(PlantData[id][PlantType] == 4)
			{
				pData[playerid][pMarijuana] += kg;
				Info(playerid, "Anda mendapatkan hasil panen marijuana/ganja seberat "LG_E2"%d kg.", kg);
			}
...
{
	if(isnull(params)) return Usage(playerid, "/farm [plant/harvest/destroy/sell]");
	...
	if(!strcmp(params, "plant", true))
	{
		if(pData[playerid][pJob] == 7 || pData[playerid][pJob2] == 7)
		{
			...
			new tstr[64];
			format(tstr, sizeof(tstr), ""WHITE_E"My Seed: "YELLOW_E"%d", pData[playerid][pSeed]);
		   	strcat(S3MP4K, "Plant\tSeed\n");
			format(String, sizeof(String), ""YELLOW_E"Potato\t"YELLOW_E"5 Seed\n");
			strcat(S3MP4K, String);
			format(String, sizeof(String), ""LIME_E"Wheat\t"YELLOW_E"18 Seed\n");
			strcat(S3MP4K, String);
			format(String, sizeof(String), ""ORANGE_E"Orange\t"YELLOW_E"11 Seed\n");
			strcat(S3MP4K, String);
			format(String, sizeof(String), ""RED_E"[ILEGAL]Marijuana\t"YELLOW_E"50 Seed\n");
			strcat(S3MP4K, String);
			ShowPlayerDialog(playerid, DIALOG_PLANT, DIALOG_STYLE_TABLIST_HEADERS, tstr, S3MP4K, "Plant", "Close");
		}
	...
	else if(!strcmp(params, "sell", true))
	{
		...
		Info(playerid, "You selling "RED_E"%d kg "WHITE_E"all plant to "LG_E"%s", total, FormatMoney(pay));
	}
	return 1;
}
...
{
	if(!IsPlayerInRangeOfPoint(playerid, 3.5, -3811.65, 1313.72, 71.42))
...
function CookingFood(playerid, type)
{
	...
	if(!IsValidTimer(pData[playerid][pCooking])) return 0;
	if(pData[playerid][pInHouse] != -1)
	{
		if(pData[playerid][pActivityTime] >= 100)
		{
