CreateJoinLumberPoint()
{
	//JOBS
	new strings[128];
	CreateDynamicPickup(1239, 23, -1438.63, -1544.56, 102.07, -1, -1, -1, 5.0);
	format(strings, sizeof(strings), "[Lumberjack Job]\n{FFFFFF}/joinjob to join");
	CreateDynamic3DTextLabel(strings, COLOR_YELLOW, -1438.63, -1544.56, 102.07, 3.0, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 1); // lumber
}
...
CMD:lumber(playerid, params[])
{
	if(pData[playerid][pJob] == 3 || pData[playerid][pJob2] == 3)
	{
		...
		if(!strcmp(params, "cut", true))
		{
			...
			{
				if(pData[playerid][pLumberTime] > 0) return Error(playerid, "You must be waiting "YELLOW_E"%d "GREY_E"second again to begin jobs.", pData[playerid][pLumberTime]);
		...
		else if(!strcmp(params, "sell")) 
		{
			// selling a log
			if(!pData[playerid][CarryingLumber]) return Error(playerid, "You're not carrying a log.");
			if(!IsPlayerInRangeOfPoint(playerid, 3.0, -1425.71, -1528.95, 102.13)) return Error(playerid, "You're not near a lumber sell "YELLOW_E"(Material Stock Crate).");
			Player_RemoveLumber(playerid);
			GivePlayerMoneyEx(playerid, LumberPrice);
			Server_MinMoney(LumberPrice);
			CrateMaterial += 10;
			pData[playerid][pLumberTime] = 1200;
			Info(playerid, "Sold a lumber for "LG_E2"%s.", FormatMoney(LumberPrice));
			// done
		}
	}
