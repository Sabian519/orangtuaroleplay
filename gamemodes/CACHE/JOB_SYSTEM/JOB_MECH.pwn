...
CreateJoinMechPoint()
{
	//JOBS
	new strings[128];
	CreateDynamicPickup(1239, 23, 2330.11, -2315.34, 13.54, -1);
	format(strings, sizeof(strings), "[MECH JOBS]\n{FFFFFF}/joinjob to join");
	CreateDynamic3DTextLabel(strings, COLOR_YELLOW, 2330.11, -2315.34, 13.54, 3.0, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 1); // Taxi
}
...
IsAtMech(playerid)
{
	...
	{
		if(IsPlayerInRangeOfPoint(playerid, 50.0, 2221.2919, -2219.0102, 13.6320)
		|| IsPlayerInRangeOfPoint(playerid, 50.0, 2215.0278, -2225.5561, 13.6320)
		|| IsPlayerInRangeOfPoint(playerid, 50.0, 2209.3271, -2232.5949, 13.6296)
		|| IsPlayerInRangeOfPoint(playerid, 50.0, 2202.2858, -2238.4267, 13.6321))
		{
			return 1;
		}
		foreach(new id : Workshop)
		{
			if(IsPlayerInRangeOfPoint(playerid, 15.0, wsData[id][wX], wsData[id][wY], wsData[id][wZ]))
		    {
...
CMD:vehiclemenu(playerid, params[])
{
	if(pData[playerid][pJob] == 2 || pData[playerid][pJob2] == 2)
	{
		...
		{
			if(!GetHoodStatus(pData[playerid][pMechVeh])) return Error(playerid,"Please open the vehicle Hood before checking services");
			...
			foreach(new id : Workshop)
			{
				if(IsPlayerInRangeOfPoint(playerid, 15.0, wsData[id][wX], wsData[id][wY], wsData[id][wZ])
				// MECHANIC PUBLIC
				|| IsPlayerInRangeOfPoint(playerid, 50.0, 2221.2919, -2219.0102, 13.6320)
				|| IsPlayerInRangeOfPoint(playerid, 50.0, 2215.0278, -2225.5561, 13.6320)
				|| IsPlayerInRangeOfPoint(playerid, 50.0, 2209.3271, -2232.5949, 13.6296)
				|| IsPlayerInRangeOfPoint(playerid, 50.0, 2202.2858, -2238.4267, 13.6321))
				{
