...
CreateJoinMinerPoint()
{
	//JOBS
	new strings[128];
	CreateDynamicPickup(1239, 23, -1919.6445,-1713.1741,21.7500, -1);
	format(strings, sizeof(strings), "[MINER JOBS]\n{FFFFFF}/joinjob to join");
	CreateDynamic3DTextLabel(strings, COLOR_YELLOW, -1919.6445,-1713.1741,21.7500, 3.0, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 1); // Miner
}
...
{	
	if(LogData[tid][logSeconds] > 1) 
	{
        ...
        new label[128];
	    format(label, sizeof(label), "Log (%d - %s)\n"WHITE_E"Dropped By "GREEN_E"%s\n"WHITE_E"%s\nUse /log pickup.", tid, typename, LogData[tid][logDroppedBy], ConvertToMinutes(LogData[tid][logSeconds]));
...
Ore_Refresh(id)
{
	...
	{
		if(!IsValidTimer(OreData[id][oreTime]))
		{
...
CMD:ore(playerid, params[])
{
	if(pData[playerid][pJob] == 5 || pData[playerid][pJob2] == 5)
	{
		...
		if(!strcmp(params, "mine", true))
		{
			...
			if(pData[playerid][pJobTime] > 0) return Error(playerid, "You must be waiting "YELLOW_E"%d "GREY_E"second again to begin jobs.", pData[playerid][pJobTime]);
		...
		else if(!strcmp(params, "sell", true))
		{
			...
			CrateComponent += LogStorage[vehicleid][0] + 25;
