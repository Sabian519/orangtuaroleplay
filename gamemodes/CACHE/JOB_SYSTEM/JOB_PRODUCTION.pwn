...
{
	if(pData[playerid][pProductionTime] > 0) return Error(playerid, "You must be waiting "YELLOW_E"%d "GREY_E"second again to begin jobs.", pData[playerid][pProductionTime]);
...
CMD:sellproduct(playerid, params[])
{
	...
	if(pData[playerid][CarryProduct] == 1)
	{
	    ...
	    AddPlayerSalary(playerid, "Production(Food)", 750);
		
		Product += 10;
		Server_MinMoney(500);
		Info(playerid, "Anda menjual 10 bahan makanan bayaran akan masuk ke paycheck/salary anda");
		pData[playerid][pProductionTime] += 1000;
	}
	else if(pData[playerid][CarryProduct] == 2)
	{
		...
		SetPlayerSpecialAction(playerid, SPECIAL_ACTION_NONE);
		pData[playerid][CarryProduct] = 0;
	    AddPlayerSalary(playerid, "Production(Clothing)", 825);
		
		Product += 10;
		Server_MinMoney(500);
		Info(playerid, "Anda menjual 10 product baju, bayaran akan masuk ke paycheck/salary anda");
		pData[playerid][pProductionTime] += 1100;
	}
	else if(pData[playerid][CarryProduct] == 3)
	{
		...
		SetPlayerSpecialAction(playerid, SPECIAL_ACTION_NONE);
		pData[playerid][CarryProduct] = 0;
	    AddPlayerSalary(playerid, "Production(Equipment)", 995);
		
		Product += 10;
		Server_MinMoney(630);
		Info(playerid, "Anda menjual 10 Product Equipment, bayaran akan masuk ke paycheck/salary anda");
		pData[playerid][pProductionTime] += 1200;
	}
	return 1;
}
