...
CMD:getcrate(playerid, params[])
{
	...
	if(MaxGetCrate[playerid] == 1000) return Error(playerid, "Pengambilan crates sudah maximal 5/5");
	// Fish
    if(IsPlayerInRangeOfPoint(playerid, 1.5, -2173.0813,632.2081,49.4375))
    {
		if(CrateFish != 0)
		{
			...
			SCM(playerid, ARWIN, "CRATE: "WHITE_E"You have "LG_E"succesfully "WHITE_E"pickup Crate's Fish with price "LG_E"$5.00"WHITE_E", put in your car use"YELLOW_E"'/loadfish'");
    ...
    if(IsPlayerInRangeOfPoint(playerid, 1.5, -1919.6656,-1702.8213,22.2700))
    {
		if(CrateComponent != 0)
		{
		    if(CarryCrate[playerid] == 1) return SCM(playerid, COLOR_YELLOW, "WARNING: "WHITE_E"Kamu sudah membawa Crates, Tidak dapat membawa lagi");
			...
			ApplyAnimation(playerid, "CARRY", "crry_prtial", 4.1, 1, 1, 1, 1, 1, 1);
			SetPlayerAttachedObject(playerid, CRATES, 2912, 1, 0.436000, 0.519000, -0.342000, 0.000000, 0.000000, 0.000000, 1.000000,1.000000,1.000000);
			SCM(playerid, ARWIN, "CRATE: "WHITE_E"You have "LG_E"succesfully "WHITE_E"pickup Crate's Component with price "LG_E"$5.00"WHITE_E", put in your car use '/loadcompo'");
	...
	// Material
    if(IsPlayerInRangeOfPoint(playerid, 1.5, -1053.0963,-664.1098,32.0126))
    {
		if(CrateMaterial != 0)
		{
		    if(CarryCrate[playerid] == 1) return SCM(playerid, COLOR_YELLOW, "WARNING: "WHITE_E"Kamu sudah membawa Crates, Tidak dapat membawa lagi");
			...
			ApplyAnimation(playerid, "CARRY", "crry_prtial", 4.1, 1, 1, 1, 1, 1, 1);
			SetPlayerAttachedObject(playerid, CRATES, 2912, 1, 0.436000, 0.519000, -0.342000, 0.000000, 0.000000, 0.000000, 1.000000,1.000000,1.000000);
			SCM(playerid, ARWIN, "CRATE: "WHITE_E"You have "LG_E"succesfully "WHITE_E"pickup Crate's Material with price "LG_E"$5.00"WHITE_E", put in your car use '/loadmats'");
...
CMD:loadcompo(playerid, params[])
{
    ...
    if(vCrateData[carid][vCrateCompo] == 1000) return Error(playerid, "There it's vehicles maximum crates stock, dont load crates again.");
...
CMD:sellcrate(playerid, params[])
{
	...
	{
		// Fish
	    if(IsPlayerInRangeOfPoint(playerid, 2.0, -1714.2910,-62.6902,3.5547))
		{
			...
			SetPlayerSpecialAction(playerid, SPECIAL_ACTION_NONE);
			RemovePlayerAttachedObject(playerid,CRATES);
			pData[playerid][pTruckerTime] = 600;
			new String[2280];
			...
			SendClientMessage(playerid, ARWIN, String);
			AddPlayerSalary(playerid, "Trucker Job(Crate Fish)", 2500);
			CarryCrate[playerid] =0;
			CarryCrateFish[playerid] =0;
		...
		// Compo
		if(IsPlayerInRangeOfPoint(playerid, 2.0, -1883.4598,-201.1821,16.0468))
		{
			...
			SendClientMessage(playerid, ARWIN, String);
			AddPlayerSalary(playerid, "Trucker Job(Crate Component)", 3000);
			CarryCrate[playerid] =0;
			CarryCrateCompo[playerid] =0;
			MaxGetCrate[playerid] =0;
			Component += 80;
		...
		// Material
		if(IsPlayerInRangeOfPoint(playerid, 2.0, -2184.5906,-209.5870,36.5156))
		{
			if(CarryCrateFish[playerid] == 1) return Error(playerid, "Kamu tidak dapat menjual Crates Fish di sini");
			...
			SendClientMessage(playerid, ARWIN, String);
			AddPlayerSalary(playerid, "Trucker Job(Crate Material)", 3500);
			CarryCrate[playerid] =0;
			CarryCrateMats[playerid] =0;
			MaxGetCrate[playerid] =0;
			Material += 50;
	...
	else return Error(playerid, "Kamu Bukan Pekerja Trucker");
	return 1;
}
...
CMD:cratemission(playerid, params[])
{
	if(pData[playerid][pJob] == 4 || pData[playerid][pJob2] == 4)
	{
		...
		strcat(location,"Crates Type\tAvailable Stock\tPrice\n",sizeof(location));
		format(lstr,sizeof(lstr), "Fish Crates\t"YELLOW_E"%d\t"LG_E"$4,50/Crate\n", CrateFish);
		strcat(location,lstr,sizeof(location));
		format(lstr,sizeof(lstr), "Component Crates\t"YELLOW_E"%d\t"LG_E"$8,00/Crate\n", CrateComponent);
		strcat(location,lstr,sizeof(location));
		format(lstr,sizeof(lstr), "Material Crates\t"YELLOW_E"%d\t"LG_E"$6,50/Crate\n", CrateMaterial);
		strcat(location,lstr,sizeof(location));
