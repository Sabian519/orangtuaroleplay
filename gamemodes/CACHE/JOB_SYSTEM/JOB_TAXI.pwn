...
CMD:taxiduty(playerid, params[])
{
	if(pData[playerid][pJob] == 1 || pData[playerid][pJob2] == 1)
	{
		...
		if(!IsEngineVehicle(vehicleid))
			return Error(playerid, "Kamu harus di dalam kendaraan.");

		if(pData[playerid][pTaxiDuty] == 0)
		{
			pData[playerid][pTaxiDuty] = 1;
			SetPlayerColor(playerid, COLOR_YELLOW);
			SendClientMessageToAllEx(COLOR_YELLOW, "[TAXI] is now on taxi duty. Type "AQUA"/call 933 "WHITE_E"to call a taxi!");
		}
		else
		{
