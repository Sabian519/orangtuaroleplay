...
Workshop_Refresh(id)
{
    if(id > -1)
    {
        ...
        format(str, sizeof str,"[WS: %d]\n"WHITE_E"Workshop Price: "YELLOW_E"%d Gold{ffffff}\n{ffffff}Type use "YELLOW_E"'/buy' "WHITE_E"to purchased this Workshop", id, wsData[id][wPrice]);

        if(strcmp(wsData[id][wOwner], "-", true))
            format(str, sizeof str,"[WS: %d]\n{ffffff}Workshop Name: "YELLOW_E"%s\n{ffffff}Workshop Owner: "AQUA"%s\n{ffffff}STATUS: %s", id, wsData[id][wName], wsData[id][wOwner], stats);
...
ShowWorkshopMenu(playerid, id)
{
    ...
    format(str, sizeof str,"Toggle Workshop\nChange Workshop Name\nEmploye Menu\nWorkshop Component\t(%d/%d)\nWorkshop Material\t(%d/%d)\nWorkshop Money\t({7fff00}%s{ffffff})",
        wsData[id][wComp],
        MAX_WORKSHOP_INT,
        wsData[id][wMat],
        MAX_WORKSHOP_INT,
...
CMD:createworkshop(playerid, params[])
{
	...
	new totalcash[25];
	format(totalcash, sizeof totalcash,"%d",price);
...
CMD:editworkshop(playerid, params[])
{
    ...
    else if(!strcmp(type, "status", true))
    {
        ...
        if(sscanf(string, "d", locked))
            return Usage(playerid, "/editWorkshop [id] [locked] [0/1]");
    ...
    else if(!strcmp(type, "price", true))
    {
        ...
        if(sscanf(string, "d", price))
            return Usage(playerid, "/editWorkshop [id] [Price] [Amount]");
	...
	else if(!strcmp(type, "money", true))
    {
        ...
        if(sscanf(string, "d", money))
            return Usage(playerid, "/editWorkshop [id] [money] [Ammount]");
	...
	else if(!strcmp(type, "comp", true))
    {
        ...
        if(sscanf(string, "d", amount))
            return Usage(playerid, "/editWorkshop [id] [Comp] [Ammount]");
    ...
    else if(!strcmp(type, "owner", true))
    {
        ...
        Workshop_Save(wid);
		Workshop_Refresh(wid);
        SendAdminMessage(COLOR_RED, "%s has adjusted the owner of Workshop ID: %d to %s", pData[playerid][pAdminname], wid, owners);
    }
