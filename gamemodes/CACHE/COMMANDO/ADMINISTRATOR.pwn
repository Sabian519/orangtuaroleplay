CMD:acmds(playerid)
{
	...
	strcat(line3, ""WHITE_E"Administrator Commands:"LB2_E"\n\
 	/aod /a /h /asay /togooc /o /goto /sendto /gethere /freeze /unfreeze /revive /spec /slap\n\
	...
	/setpassword /createhouse /edithouse /createbisnis /editbisnis /creategym /editgym");
 	
	strcat(line3, "\n"BLUE_E"OT:RP "WHITE_E"- Anti-Cheat is actived.\n\
	"PINK_E"NOTE: All admin commands log is saved in database! | Abuse Commands? Kick And Demote Premanent!.");
	ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, ""ORANGE_E"OT:RP: "YELLOW_E"Staff Commands", line3, "OK","");
	return true;
}
...
CMD:hcmds(playerid)
{
	...
	strcat(line3, ""WHITE_E"Junior Helper Commands:"LB2_E"\n\
 	/aod /h /asay /o /goto /sendto /gethere /freeze /unfreeze\n\
...
CMD:asalve(playerid, params[])
{
	...
	new otherid;
	if(sscanf(params, "u", otherid))
        return Usage(playerid, "/asalve [playerid/PartOfName]");
    ...
    Info(otherid, "Admin %s has resalve your sick character.", ReturnName(playerid));
	return 1;
}
...
CMD:jetpack(playerid, params[])
{
    ...
    if(pData[playerid][pAdminDuty] < 1)
        return Error(playerid, "You can access this while being Admin On Duty");
    ...
    if(sscanf(params, "u", otherid))
    {
        ...
        Info(playerid, "You have used jetpack, '"YELLOW_E"/deljp"WHITE_E"' to removed jetpack");
    }
    else
    {
        //pData[playerid][pJetpack] = 1;
...
CMD:ban(playerid, params[])
{
    ...
    if(pData[playerid][pAdminDuty] < 1)
        return Error(playerid, "You can access this while being Admin On Duty /aod");
...
CMD:respawnsapd(playerid, params[])
{
	...
	for(new x;x<sizeof(SAPD_Garages);x++)
	{
		if(IsVehicleEmpty(SAPD_Garages[x]))
		{
			SetVehicleToRespawn(SAPD_Garages[x]);
			SetValidVehicleHealth(SAPD_Garages[x], 2000);
			SetVehicleFuel(SAPD_Garages[x], 1000);
			SwitchVehicleDoors(SAPD_Garages[x], false);
		}
	}
...
CMD:respawnjobs(playerid, params[])
{
	...
	for(new x;x<sizeof(BUS);x++)
	{
		if(IsVehicleEmpty(BUS[x]))
		{
			SetVehicleToRespawn(BUS[x]);
			SetValidVehicleHealth(BUS[x], 2000);
			SetVehicleFuel(BUS[x], 1000);
			SwitchVehicleDoors(BUS[x], false);
		}
	}
	for(new xx;xx<sizeof(FORKLIFT);xx++)
	{
		if(IsVehicleEmpty(FORKLIFT[xx]))
		{
			SetVehicleToRespawn(FORKLIFT[xx]);
			SetValidVehicleHealth(FORKLIFT[xx], 2000);
			SetVehicleFuel(FORKLIFT[xx], 1000);
			SwitchVehicleDoors(FORKLIFT[xx], false);
		}
	}
	for(new xx;xx<sizeof(SWEEPER);xx++)
	{
		if(IsVehicleEmpty(SWEEPER[xx]))
		{
			SetVehicleToRespawn(SWEEPER[xx]);
			SetValidVehicleHealth(SWEEPER[xx], 2000);
			SetVehicleFuel(SWEEPER[xx], 1000);
			SwitchVehicleDoors(SWEEPER[xx], false);
		}
	}
	for(new xx;xx<sizeof(pizzabikes);xx++)
	{
		if(IsVehicleEmpty(pizzabikes[xx]))
		{
			SetVehicleToRespawn(pizzabikes[xx]);
			SetValidVehicleHealth(pizzabikes[xx], 2000);
			SetVehicleFuel(pizzabikes[xx], 1000);
			SwitchVehicleDoors(pizzabikes[xx], false);
		}
	}
...
CMD:setcs(playerid, params[])
{
	...
	new targetid, option;
	if(sscanf(params,"ui", targetid, option)) return Usage(playerid, "/setcs [player targetid or name] [0 = none 1 = approved]");
	if(!IsPlayerConnected(targetid)) return Error(playerid, "There is player not connect");
	Servers(playerid, "Kamu telah mengset Character Story "X1_BLUE"%s "YELLOW_E"menjadi = %s", ReturnName(targetid), option);
	Servers(targetid, "Admin "X1_BLUE"%s "YELLOW_E"telah mengset Character Story kamu menjadi menjadi = %s", ReturnName(playerid), option);
	pData[targetid][pCharacterStory] = option;
	return 1;
}
...
CMD:resetalldelays(playerid, params[])
{
    if(pData[playerid][pAdmin] < 3)
        return PermissionError(playerid);
	...
	pData[playerid][pTruckerTime] = 0;
	pData[playerid][pLumberTime] = 0;
	pData[playerid][pProductionTime] = 0;
	pData[playerid][pDepositorTime] = 0;
 	pData[playerid][pSideJobTime] = 0;
 	pData[playerid][pBusTime] = 0;
 	pData[playerid][pForTime] = 0;
	pData[playerid][pPizzaTime] = 0;
	pData[playerid][pSmugglerTime] = 0;
	pData[playerid][pMission] = -1;
	pData[playerid][pHauling] = -1;
	pData[playerid][pDealerMission] = -1;
	Info(playerid, "You haved restarted delays jobs");
	return 1;
}
...
CMD:setmoney(playerid, params[])
{
	...
	if(money < 0 || money > 100000000)
 		return Error(playerid, "Maximum inset ammount $700,00.");
...
CMD:givemoney(playerid, params[])
{
	...
	if(money < 0 || money > 20000)
 		return Error(playerid, "Maximum inset ammount $200,00.");
...
CMD:setmn(playerid, params[])
{
	...
	if(sscanf(params, "ud", otherid, money))
	{
	    Usage(playerid, "/setmn <ID/Name> <money>");
	    return true;
	}
...
CMD:setmaterial(playerid, params[])
{
	...
	if(money < 0 || money > 1000)
 		return Error(playerid, "Maximum inset ammount 1.000.");
...
CMD:setmarijuana(playerid, params[])
{
	...
	format(tmp, sizeof(tmp), "%d", money);
	StaffCommandLog("SETMARI", playerid, otherid, tmp);
	return 1;
}
...
CMD:setlevel(playerid, params[])
{
	...
	if(jumlah < 1 || jumlah > 5)
 		return Error(playerid, "Maximum inset level 5.");
...
CMD:setlv(playerid, params[])
{
	...
	new jumlah, otherid;
	if(sscanf(params, "ud", otherid, jumlah))
        return Usage(playerid, "/setlv [playerid id/name] <jumlah>");
...
CMD:setcomponent(playerid, params[])
{
	...
	if(money < 0 || money > 100000)
 		return Error(playerid, "Maximum inset ammount 1.000,000.");
...
CMD:setg(playerid, params[])
{
	...
	if(sscanf(params, "ud", otherid, gold))
	{
	    Usage(playerid, "/setg <ID/Name> <gold>");
	    return true;
	}
...
CMD:giveg(playerid, params[])
{
	...
	if(sscanf(params, "ud", otherid, gold))
	{
	    Usage(playerid, "/giveg <ID/Name> <gold>");
	    return true;
	}
...
CMD:setpassword(playerid, params[])
{
	...
	SCM(playerid, COLOR_RED, "Admin %s You have succesfully change Account Player");
	return 1;
}
